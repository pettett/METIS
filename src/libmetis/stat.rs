...
pub unsafe extern "C" fn libmetis__ComputePartitionInfoBipartite(
    ...
    if vwgt.is_null() {
        ...
        vwgt = (*graph).vwgt;
        mustfree = 1 as libc::c_int;
    }
    if adjwgt.is_null() {
        (*graph)
            .adjwgt = libmetis__ismalloc(
            *xadj.offset(nvtxs as isize) as size_t,
            1 as libc::c_int,
    ...
    kpwgts = libmetis__ismalloc(
        ...
        0 as libc::c_int,
        b"ComputePartitionInfo: kpwgts\0" as *const u8 as *const libc::c_char
            as *mut libc::c_char,
    );
    i = 0 as libc::c_int;
    while i < nvtxs {
        j = 0 as libc::c_int;
        while j < ncon {
            let ref mut fresh0 = *kpwgts
                .offset((*where_0.offset(i as isize) * ncon + j) as isize);
    ...
    if ncon == 1 as libc::c_int {
        printf(
            ...
            1.0f64 * nparts as libc::c_double
            ...
            1.0f64 * nparts as libc::c_double
                * *vwgt.offset(libmetis__iargmax(nvtxs as size_t, vwgt) as isize)
                    as libc::c_double
                / (1.0f64
                    * libmetis__isum(
                        nparts as size_t,
    ...
    padjncy = libmetis__ismalloc(
        ...
        0 as libc::c_int,
        b"ComputePartitionInfo: padjncy\0" as *const u8 as *const libc::c_char
            as *mut libc::c_char,
    );
    padjwgt = libmetis__ismalloc(
        (nparts * nparts) as size_t,
        0 as libc::c_int,
        b"ComputePartitionInfo: padjwgt\0" as *const u8 as *const libc::c_char
            as *mut libc::c_char,
    );
        ...
        while j < *xadj.offset((i + 1 as libc::c_int) as isize) {
                ...
                != *where_0.offset(*adjncy.offset(j as isize) as isize)
            {
                ...
                *fresh1 += *adjwgt.offset(j as isize);
                if *kpwgts
                    .offset(
                        *where_0.offset(*adjncy.offset(j as isize) as isize) as isize,
                    ) == 0 as libc::c_int
                {
                    let ref mut fresh2 = *padjwgt
                        .offset(
                            (*where_0.offset(i as isize) * nparts
                                + *where_0.offset(*adjncy.offset(j as isize) as isize))
                                as isize,
    ...
    printf(
        ...
        *kpwgts.offset(libmetis__iargmax(nparts as size_t, kpwgts) as isize),
        libmetis__isum(nparts as size_t, kpwgts, 1 as libc::c_int as size_t) / nparts,
        1.0f64 * nparts as libc::c_double
            * *kpwgts.offset(libmetis__iargmax(nparts as size_t, kpwgts) as isize)
                as libc::c_double
            / (1.0f64
                * libmetis__isum(nparts as size_t, kpwgts, 1 as libc::c_int as size_t)
                    as libc::c_double),
    );
    i = 0 as libc::c_int;
    while i < nparts {
    ...
    printf(
        ...
        *kpwgts.offset(libmetis__iargmax(nparts as size_t, kpwgts) as isize),
        ...
        1.0f64
            * libmetis__isum(nparts as size_t, kpwgts, 1 as libc::c_int as size_t)
                as libc::c_double / (1.0f64 * nvtxs as libc::c_double),
    );
    if mustfree == 1 as libc::c_int || mustfree == 3 as libc::c_int {
        gk_free(
...
pub unsafe extern "C" fn libmetis__ComputePartitionBalance(
    ...
    if vwgt.is_null() {
        ...
        *ubvec
            ...
            * *kpwgts.offset(libmetis__iargmax(nparts as size_t, kpwgts) as isize)
                as libc::c_double / (1.0f64 * nvtxs as libc::c_double)) as real_t;
    } else {
        j = 0 as libc::c_int;
        while j < ncon {
            libmetis__iset(nparts as size_t, 0 as libc::c_int, kpwgts);
...
pub unsafe extern "C" fn libmetis__ComputeElementBalance(
    ...
    kpwgts = libmetis__ismalloc(
        nparts as size_t,
        0 as libc::c_int,
        b"ComputeElementBalance: kpwgts\0" as *const u8 as *const libc::c_char
            as *mut libc::c_char,
    );
    i = 0 as libc::c_int;
    while i < ne {
    ...
    balance = (1.0f64 * nparts as libc::c_double
        * *kpwgts.offset(libmetis__iargmax(nparts as size_t, kpwgts) as isize)
            as libc::c_double
        / (1.0f64
            * libmetis__isum(nparts as size_t, kpwgts, 1 as libc::c_int as size_t)
